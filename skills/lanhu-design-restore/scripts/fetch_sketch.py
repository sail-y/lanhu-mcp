...
def _find_repo():
    ...
    if (candidate / 'lanhu_mcp_server.py').exists() and (candidate / 'lanhu' / 'tools').exists():
        return candidate
    # 2. 环境变量显式指定
    env = os.environ.get('LANHU_MCP_REPO')
    if env:
        p = Path(env)
        if (p / 'lanhu_mcp_server.py').exists():
            return p
    # 3. 常见路径兜底
    home = Path.home()
    for p in [
...
def main():
    ...
    candidates = [
        server_path,
        str(_find_repo() / 'lanhu_mcp_server.py'),
    ]
